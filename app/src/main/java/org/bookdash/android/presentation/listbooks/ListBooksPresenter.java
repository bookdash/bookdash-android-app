...
class ListBooksPresenter extends BasePresenter<ListBooksContract.View> implements ListBooksContract.Presenter {
    ...
    ListBooksPresenter(ListBooksContract.View listBooksView, SettingsRepository settingsRepository, BookService bookService, Scheduler ioScheduler, Scheduler mainScheduler) {
    ...
    public void loadLanguages() {
        addSubscription(bookService.getLanguages().subscribe(new Action1<List<FireLanguage>>() {
            @Override
    ...
    public void saveSelectedLanguage(final int indexOfLanguage) {
        addSubscription(settingsRepository.saveLanguagePreference(languages.get(indexOfLanguage)).observeOn(ioScheduler).subscribeOn(mainScheduler).subscribe(new Subscriber<Boolean>() {
            @Override
    ...
    public void loadBooksForLanguagePreference() {
        addSubscription(settingsRepository.getLanguagePreference().observeOn(ioScheduler).subscribeOn(mainScheduler).subscribe(new Subscriber<FireLanguage>() {
            @Override
            public void onCompleted() {
    ...
    private void loadBooksForLanguage(FireLanguage language) {
        ...
        addSubscription(bookService.getBooksForLanguage(language).subscribe(new Action1<List<FireBookDetails>>() {
            @Override
